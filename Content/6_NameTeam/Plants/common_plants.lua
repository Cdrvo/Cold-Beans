...
SMODS.Joker({
	...
	calculate = function(self, card, context)
		...
		if
			...
			and context.destroy_card == G.play.cards[1]
		then
...
SMODS.Joker({
	...
	calculate = function(self, card, context)
		...
		if
			context.individual
			and context.cardarea == G.play
			and context.other_card == NAMETEAM.find_highest(context.scoring_hand)
		then
			return {
				mult = cae.mult,
			}
...
SMODS.Joker({
	...
	loc_vars = function(self, info_queue, card)
		...
		return {
			vars = {
				cae.dollars,
				(cae.dollars + ((G.jokers and NAMETEAM.pool_in("cbean_shadow", (G.jokers.cards or ""))) or 0)),
			},
		}
	end,
	calc_dollar_bonus = function(self, card)
...
SMODS.Joker({
	...
	calculate = function(self, card, context)
		...
		if
			...
			and G.blind_select
			and Colonparen.get_blind_by_key(G.GAME.round_resets.blind_choices["Boss"]).key ~= "bl_cbean_blank"
		then
...
SMODS.Joker({
	...
	calculate = function(self, card, context)
		...
		if
			context.individual
			and context.cardarea == G.play
			and context.other_card == NAMETEAM.find_highest(G.play.cards)
		then
			return {
				xmult = cae.exxmult,
				card = card,
				...
				func = function()
					...
					acard:set_ability(
						pseudorandom_element(
							{ "m_cbean_basic_zomboid", "m_cbean_carrot_zomboid", "m_cbean_melon_zomboid" },
							pseudoseed("znake_lilly")
						)
					)
					return true
				end,
