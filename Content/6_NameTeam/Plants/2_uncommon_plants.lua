...
SMODS.Joker({
	...
	calculate = function(self, card, context)
		...
		if context.selling_self then
			...
			if not G.GAME.NAMETEAM.reduce then
				G.GAME.NAMETEAM.reduce = 0
			end
...
SMODS.Joker({
	...
	calculate = function(self, card, context)
		...
		if
			...
			and SMODS.pseudorandom_probability(card, "snowpea_seedssomething", 1, cae.odds)
		then
...
SMODS.Joker({
	...
	calculate = function(self, card, context)
		...
		if context.destroy_card and context.cardarea == G.play and not context.blueprint and cae.active then
			local a = pseudorandom_element(context.scoring_hand, "seed_jhere")
			-- print(a, context.destroy_card)
			if context.destroy_card == a then
				cae.active = false
				cae.active_this_round = true
				cae.rounds_current = 3
				return {
					remove = true,
	...
	calculate = function(self, card, context)
		...
		if context.before and not context.blueprint then
			...
			for i = 1, #G.jokers.cards do
				if
					i > cae.rr
					and (string.find(G.jokers.cards[i].config.center.key, "shroom") or card.ability.cbean_shroom)
				then
					...
					for k, v in pairs(context.other_ret) do
						if type(v) == "table" then
							for k, v in pairs(v) do
								...
								if
									string.find(a, "CHIPS")
									or string.find(a, "CHIPS_MOD")
									or string.find(a, "CHIP_MOD")
								then -- i had this ready so i coyp pasted
		...
		if context.before and not context.blueprint then
			...
			for i = 1, #G.jokers.cards do
				if i > cae.rr then
					...
					G.jokers.cards[i].ability.extra.electric_reeded_num =
						NAMETEAM.perc(G.jokers.cards[i]:on_the("left").ability.extra.electric_reeded_num, 75)
				end
			end
		end
...
SMODS.Joker({
	...
	calculate = function(self, card, context)
		...
		if
			...
			and (context.other_context.joker_main or context.other_context.individual or NAMETEAM.during_scoring)
		then
			if card:on_the("left") and context.other_card == card:on_the("left") then
				cae.go = true
			end
		end
		if
...
SMODS.Joker({
	...
	calculate = function(self, card) end,
	remove_from_deck = function(self, card, from_debuff)
		local cae = card.ability.extra
		cae.stop = true
		for k, v in pairs(G.playing_cards) do
...
SMODS.Joker({
	...
	loc_vars = function(self, info_queue, card)
		info_queue[#info_queue + 1] = G.P_TAGS.tag_cbean_sunbean
		local cae = card.ability.extra
		return { vars = { cae.dollars } }
	end,
	calculate = function(self, card, context)
	...
	loc_vars = function(self, info_queue, card)
		...
		return {
			vars = {
				...
				).mult * G.GAME.starting_params.ante_scaling) / 2) or 0,
			},
		}
	end,
	calculate = function(self, card, context)
		...
		if context.selling_self and G.GAME.blind and G.GAME.blind.in_blind then
			local blind_amt = (
				...
				* Colonparen.get_blind_by_key(G.GAME.round_resets.blind_choices["Small"]).mult
...
SMODS.Joker({
	...
	calculate = function(self, card, context)
		...
		if context.individual and context.cardarea == G.play then
			...
			if c == gp[1+1+1] and gp[1+1+1] then
				return {
					xmult = cae.xmult,
				}
			end
...
SMODS.Joker({
	...
	calculate = function(self, card, context)
		...
		if
			...
			and context.destroy_card == context.scoring_hand[1]
			and not context.blueprint
		then
...
SMODS.Joker({
	...
	calculate = function(self, card, context)
		...
		if
			...
			and context.cardarea == G.play
			and cae.small_card
			and context.other_card == cae.small_card
		then
			return {
				xmult = cae.xmult,
			}
		...
		if card.added_to_deck then
			for k, v in pairs(G.jokers.cards) do
				if
					...
					and v.config.center.pools.cbean_pea
					and not v.ability["pead_" .. cae.key]
					and v.config.center.key ~= "j_cbean_pea_vine"
				then
...
SMODS.Joker({
	...
	calculate = function(self, card, context)
		...
		if
			...
			and cae.go
			and (
				card:on_the("left") and context.other_card == card:on_the("left")
				or card:on_the("right") and context.other_card == card:on_the("right")
			)
		then
			return {
				repetitions = cae.rep2,
			...
			for k, v in pairs(G.jokers.cards) do
				if v ~= card then
					if
						not (v.sell_cost <= 0)
						and (
							not v.ability.extra_value
							or (v.ability.extra_value and (v.ability.extra_value + v.sell_cost >= 0))
						)
					then
	...
	calculate = function(self, card, context)
		...
		if context.after and not context.blueprint then
			if cae.round_timer > 1 then
				...
				local acard = SMODS.add_card{
					set = "Enhanced",
					enhancement = pseudorandom_element({"m_cbean_basic_zomboid","m_cbean_carrot_zomboid","m_cbean_melon_zomboid"}),
					area = G.deck
				}
			end
		end
