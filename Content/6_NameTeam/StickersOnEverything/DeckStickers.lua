function Back:nteam_apply_sticker(key)
	...
	if trigger_add then
		...
		SMODS.calculate_context({sticker_applied = true, other_sticker = sticker_table, other_card = self, is_deck_sticker = true})
	end
end
...
function Back:nteam_remove_sticker(key)
	...
	if trigger_remove then
		...
		SMODS.calculate_context({sticker_removed = true, other_sticker = sticker_table, other_card = self, is_deck_sticker = true})
	end
end
