...
-- changebling function(blindType: string, before: blindobject, after: blindobject)
    if not G.blind_select_opts[before] then error("Could not update blind, as its not in current blinds", 1) return end
    G.E_MANAGER:add_event(Event({ -- Lower
        trigger = 'immediate',
        func = function()
          play_sound('other1')
          G.blind_select_opts[before]:set_role({xy_bond = 'Weak'})
